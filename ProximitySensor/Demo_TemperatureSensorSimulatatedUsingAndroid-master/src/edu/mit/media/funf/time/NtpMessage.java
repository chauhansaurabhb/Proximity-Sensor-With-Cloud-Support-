 ...
 *
 * This class does not support the optional authentication protocol, and ignores
 * the key ID and message digest fields.
 * 
 * For convenience, this class exposes message values as native Java types, not
 * the NTP-specified data formats. For example, timestamps are stored as doubles
 * (as opposed to the NTP unsigned 64-bit fixed point format).
 * 
 * However, the contructor NtpMessage(byte[]) and the method toByteArray() allow
 * the import and export of the raw NTP message format.
 * 
 * 
 * Usage example
 * 
 * // Send message DatagramSocket socket = new DatagramSocket(); InetAddress
 * address = InetAddress.getByName("ntp.cais.rnp.br"); byte[] buf = new
 * NtpMessage().toByteArray(); DatagramPacket packet = new DatagramPacket(buf,
 * buf.length, address, 123); socket.send(packet);
 * 
